...
import UrlScreen from '../screens/authScreen/UrlScreen';
...
function AppNavigation() {
    ...
    const fetchConfiguration = async (url: string) => {
        try {
            ...
            if (configDetails) {
                await AsyncStorage.setItem('configDetails', JSON.stringify(configDetails));  // Save config details in AsyncStorage
    ...
    useEffect(() => {
        if (baseUrls) {
            ...
            store.dispatch(GetBuinessUnits({ baseUrl: baseUrls, buCode: buCode, token: token }));  // Dispatch action to get business units
        }
    }, [baseUrls, dispatch]);
