...
import { getRfidListAction, deleteRfidListAction } from '../../reducer/RFIDList/RFIDListAction';
...
export const RfidListHook = () => {
  ...
  const ListData = useSelector((state: RootState) => state.rfidList.RfidListData);
  ...
  const DError = useSelector((state: RootState) => state.rfidList.errors.delete);
  ...
  const loadRfidList = useCallback(async () => {
    setRefreshing(true);
    await dispatch(getRfidListAction({ baseUrl }));
    setRefreshing(false);
  }, [baseUrl, dispatch]);
      ...
      Alert.alert(
        ...
        [
          { text: 'Cancel', style: 'cancel' },
          {
            text: 'OK',
            onPress: async () => {
              const resultAction = await dispatch(deleteRfidListAction({ id, buCode, token }));
              if (deleteRfidListAction.fulfilled.match(resultAction)) {
                Alert.alert('Success', 'RFID deleted successfully!', [{ text: 'OK' }], { cancelable: false });
