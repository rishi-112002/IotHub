...
const handleRejected = (
  ...
  state.errors[actionType] = errorMessage;

  // Log unexpected errors
  if (!action.payload) {
    console.error('Unhandled rejection:', action);
  }
};
...
const addAsyncCases = (
  ...
  action: any,
  actionType: keyof AuthState['errors'],
  fulfilledCallback: (state: AuthState, action: PayloadAction<any>) => void
) => {
  builder.addCase(action.fulfilled, (state: AuthState, action: PayloadAction<any>) => {
    fulfilledCallback(state, action);
    state.loader = false;
    state.errors[actionType] = null;
  });
  builder.addCase(action.rejected, (state: AuthState, action: PayloadAction<any>) => {
    handleRejected(state, actionType, action);
  });
  builder.addCase(action.pending, (state: AuthState) => {
    handlePending(state, actionType);
  });
...
export const rfidListSlice = createSlice({
  ...
  extraReducers: builder => {
    ...
    addAsyncCases(
      ...
      CreateRFIDdata,
      'create',
      (state: AuthState, action: PayloadAction<RfidData>) => {
        state.RfidListData.push(action.payload);
      },
    );
    ...
    addAsyncCases(
      ...
      deleteRfidListAction,
      'delete',
      (state: AuthState, action: PayloadAction<{ id: string }>) => {
        state.RfidListData = state.RfidListData.filter(
          item => item.id !== action.payload.id,
        );
      },
